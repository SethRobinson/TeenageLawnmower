...
bool CTextParse::get_next_line(char **pointer_to_buff)
{
	...
	} else
	{
			while (ui_cur_pos < ui_buffer_size)
			{
                ...
                if (i_position > m_i_line_buffer_size)
                {
                    //um, we got a problem..
                    LogError(_T("Line too big, let's realocate the buffer"));
                    m_i_line_buffer_size *= 2;
                    p_buffer = (char*)realloc(p_buffer, m_i_line_buffer_size);
                    if (!p_buffer)
                    {
...
{
	p_word[0] = NULL;
	seperate_string(p_line, i_word_num, ch_sep, p_word);
   return p_word;
}

//items must be separated with | char
char * CTextParse::get_word(int i_word_num)
{
	p_word[0] = NULL;
	seperate_string(p_line, i_word_num, '|', p_word);
 /*
	token = strtok( p_buffer, "|" );
  int counter = 0;
  while( token != NULL )
   {
  	  counter++;
   if (counter == i_word_num)
	  {
		  Msg("Returning %s - %d",token, i_word_num);
		  strcpy(p_word, token);
		return p_word;
	  }
	    token = strtok( NULL, "|" );
