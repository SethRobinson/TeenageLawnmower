...
void CScriptVarBase::SetVar(int i_type, int i_parm1, int i_parm2, void *p_extra)
{
   ...
   if (i_type == ::C_SCRIPT_VAR_WCHAR)
   {
	   ...
	   {
		//also set a default
		int i_length = wcslen((unsigned short*)p_extra);
     ...
     if (i_type == ::C_SCRIPT_VAR_VECTOR3)
   {
	   //making a string resource
	   m_p_extra = realloc(m_p_extra, 12); //alloc or realloc whatever was there if it's not NULL
	  
	   if (p_extra)
	   {
	   *((D3DXVECTOR3*)m_p_extra) = *((D3DXVECTOR3*)p_extra);
	   } else
	   {
		   //it's null, let's just add zeroes for now.
		  memset(m_p_extra,0, 12);
	   }
...
bool CScriptVarBase::Modify(CScriptVarBase *p_new, int i_mod_type)
{
   ...
   if (p_new->GetType() == C_SCRIPT_VAR_DYNAMIC)
   {
   	...
   	if (this->GetType() == p_new->GetType())
	{
		...
		{
			if (i_mod_type == ::C_SCRIPT_OP_ADD) SetVector3( &(*GetVector3()+*p_new->GetVector3())); else
		
				
			if (i_mod_type == ::C_SCRIPT_OP_SUBTRACT)  SetVector3( &( (*GetVector3()) - (*p_new->GetVector3()))); else
...
int CScriptVarBase::AppendToWChar(WCHAR *p_dest)
{
   //let's convert whatever our var type is into string format.
	...
	if (GetType() == ::C_SCRIPT_VAR_WCHAR)
	{
		...
		if (GetType() == ::C_SCRIPT_VAR_VECTOR3)
	{
	

		return (swprintf(p_dest, L"X: %.4f Y: %.4f Z: %.4f", ((D3DXVECTOR3*)m_p_extra)->x, ((D3DXVECTOR3*)m_p_extra)->y, ((D3DXVECTOR3*)m_p_extra)->z));
...
bool CScriptVarBase::If(CScriptVarBase *p_new, int i_mod_type)
{

 	if (GetType() == C_SCRIPT_VAR_DYNAMIC)
	{
		//let's actually change the var we should be pointing too
		return g_p_script_engine->GetScript(this->m_i_parm1)->GetVar(m_i_parm2)->If(p_new, i_mod_type);
	}
	
   if (p_new->GetType() == C_SCRIPT_VAR_DYNAMIC)
   {
	   //call us with the real var, not this crap
	   return If(g_p_script_engine->GetScript(p_new->m_i_parm1)->GetVar(p_new->m_i_parm2), i_mod_type);
   }
   ...
   // assert(0);
    //if we got here there is an error
	LogError("Don't know how to compare %s to a %s in IF statement.", GetVarTypeString(GetType()), GetVarTypeString(p_new->GetType()));
