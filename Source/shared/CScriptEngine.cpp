...
void func_make_vector3 (CScriptPassVars *vars)
{
	vars->return_var.SetVar(C_SCRIPT_VAR_VECTOR3,0,0, &D3DXVECTOR3(vars->a_vars[0].GetFloat(), vars->a_vars[1].GetFloat(),
		vars->a_vars[2].GetFloat()));
}


void func_vector3getx (CScriptPassVars *vars)
{
	vars->return_var.SetVar(C_SCRIPT_VAR_FLOAT, *(int*) &vars->a_vars[0].GetVector3()->x,0,NULL);
}

void func_vector3gety (CScriptPassVars *vars)
{
	vars->return_var.SetVar(C_SCRIPT_VAR_FLOAT, *(int*) &vars->a_vars[0].GetVector3()->y,0,NULL);
}
void func_vector3getz (CScriptPassVars *vars)
{
	vars->return_var.SetVar(C_SCRIPT_VAR_FLOAT, *(int*) &vars->a_vars[0].GetVector3()->z,0,NULL);
}

void func_vector3mody (CScriptPassVars *vars)
{

   D3DXVECTOR3 vec3 = *vars->a_vars[0].GetVector3();
      vec3.y += vars->a_vars[1].GetFloat();
    vars->return_var.SetVar(C_SCRIPT_VAR_VECTOR3,0,0, &vec3);
}
...
void CScriptEngine::InitGlobalScript()
{
	...
	p_vars->return_var.SetType(C_SCRIPT_VAR_VECTOR3);
	script_vec[0]->AddFunction("MakeVector3", func_make_vector3, p_vars);


	p_vars = new CScriptPassVars();
	p_vars->a_vars[0].SetType(C_SCRIPT_VAR_VECTOR3);
	p_vars->return_var.SetType(C_SCRIPT_VAR_FLOAT);
	script_vec[0]->AddFunction("Vector3GetX", func_vector3getx, p_vars);

	p_vars = new CScriptPassVars();
	p_vars->a_vars[0].SetType(C_SCRIPT_VAR_VECTOR3);
	p_vars->return_var.SetType(C_SCRIPT_VAR_FLOAT);
	script_vec[0]->AddFunction("Vector3GetY", func_vector3gety, p_vars);

	p_vars = new CScriptPassVars();
	p_vars->a_vars[0].SetType(C_SCRIPT_VAR_VECTOR3);
	p_vars->return_var.SetType(C_SCRIPT_VAR_FLOAT);
	script_vec[0]->AddFunction("Vector3GetZ", func_vector3getz, p_vars);

 	p_vars = new CScriptPassVars();
	p_vars->a_vars[0].SetType(C_SCRIPT_VAR_VECTOR3);
	p_vars->a_vars[1].SetType(C_SCRIPT_VAR_FLOAT);
	p_vars->return_var.SetType(C_SCRIPT_VAR_VECTOR3);
	script_vec[0]->AddFunction("Vector3ModY", func_vector3mody, p_vars);
