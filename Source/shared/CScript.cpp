...
void CScript::Kill()
{
	...
	for (int i=0; i < m_function_vec.size(); i++)
	{
    ...
    for (int v=0; v < m_var_vec.size(); v++)
	{
...
int CScript::FindVarByName(const char *st_search)
{
  ...
  //      LogMsg("HASHed var %d found", i_hash_index);
    ...
    for (int i=0; i < m_var_vec.size(); i++)
...
//		LogMsg("Loading %s...", st_var_name);
		...
		{
			D3DXVECTOR3 vec_value;
			fread(&vec_value, sizeof(D3DXVECTOR3), 1, fp);
			if (i_var_index != -1)
			{
				//set it.
				m_var_vec[i_var_index]->SetVector3(&vec_value);
			}
