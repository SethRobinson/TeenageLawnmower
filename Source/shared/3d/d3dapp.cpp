...
CD3DApplication::CD3DApplication()
{
    ...
    m_strWindowTitle    = _T("RTSOFT_ENGINE");
...
b_force_mode = false;
m_b_vsyc = false; //false is good, don't limit the drawing to the refresh rate
m_clear_flags =  D3DCLEAR_ZBUFFER;
m_bInitVideoOnNextResize = false;
m_b_cursor_is_on = true;
                      
}



//insure ShowCursor ref count is done correctly so we can call it willy nilly
void CD3DApplication::SetShowCursor(bool b_new)
{
    if (b_new != m_b_cursor_is_on)
    {
            ShowCursor(b_new);
           m_b_cursor_is_on = b_new;
      }
}
//-----------------------------------------------------------------------------
// Name: WndProc()
...
HRESULT CD3DApplication::Create( HINSTANCE hInstance )
{
    ...
    //                      WS_MINIMIZEBOX|WS_VISIBLE;
       m_dwWindowStyle = WS_OVERLAPPED|WS_CAPTION|WS_SYSMENU|WS_MAXIMIZEBOX| WS_THICKFRAME|
                          WS_MINIMIZEBOX|WS_VISIBLE;
        ...
        for( UINT iDevice = 0; iDevice < dwNumDeviceTypes; iDevice++ )
        {
            ...
            for( DWORD f=0; f<dwNumFormats; f++ )
            {
                ...
                if( pDevice->d3dCaps.DevCaps&D3DDEVCAPS_HWTRANSFORMANDLIGHT )
                {
                    ...
                    {
                       	//Seth
						//erasing this pure crap so NVidia cards work with the Get() command
            ...
            for( DWORD m=0; m<pDevice->dwNumModes; m++ )
            {
				...
				if( pDevice->modes[m].Width==i_force_width && pDevice->modes[m].Height==i_force_height )
				 {
                    pDevice->dwCurrentMode = m;
                    if( pDevice->modes[m].Format == this->d3d_force_depth)
                    {
 						//this matches exactly what we wanted for our initial res and depth!
						//why look farther? exit
						break;
                    }
                }
                                  ...
                                  LPARAM lParam )
{
    ...
    switch( uMsg )
    {
        ...
        case WM_SIZE:
            ...
            {
                if( m_bActive && m_bWindowed && m_bInitVideoOnNextResize )
                {
                    m_bInitVideoOnNextResize = false;
                    RECT rcClientOld;
...
HRESULT CD3DApplication::Initialize3DEnvironment()
{
    ...
    else
    {
        ...
        if (!m_b_vsyc)
        {
            ...
            if (pDeviceInfo->d3dCaps.PresentationIntervals & D3DPRESENT_INTERVAL_IMMEDIATE)
            {
...
void CD3DApplication::SetupStartMode(bool b_windowed, int i_w, int i_h, D3DFORMAT d3d_depth)
{
	this->b_force_mode = true;
    m_bWindowed = b_windowed;
	i_force_width = i_w;
	i_force_height = i_h;
	d3d_force_depth = d3d_depth;
...
HRESULT CD3DApplication::ReinitializeTheDevice()
{
    HRESULT hr;
    m_bReady = FALSE;
...
{
    HRESULT hr;
     m_b_remember_windowed = m_bWindowed != 0;
                                                    ...
                                                    WPARAM wParam, LPARAM lParam )
{
    ...
    if( WM_INITDIALOG == msg )
    {
      ...
      //  bOldWindowed = pd3dApp->m_b_remember_windowed;
        ...
        dwNewMode    = dwOldMode;
        bNewWindowed = pd3dApp->m_b_remember_windowed;
...
INT CD3DApplication::Run()
{
    ...
    while( WM_QUIT != msg.message  )
    {
        ...
        if( bGotMsg )
        {
            ...
            {
                //This gets triggered when alt-entering and gives "this software requires a newer version of windows"
                //errors.  Strange but harmless?
                //LogLastError();
                //assert(0 && "TranslateAccelerator error?");
            }
...
HRESULT CD3DApplication::Render3DEnvironment()
{
    ...
    if( FAILED( hr = m_pd3dDevice->TestCooperativeLevel() ) )
    {
        ...
        {
          	log_msg("Device not reset, setting it up.");
