...
public:
	...
	bool b_force_mode; //if true we'll use Seth's funky mode forcing
	DWORD i_force_height, i_force_width;
	D3DFORMAT d3d_force_depth;
...
public:
	...
	bool b_mfc; //Added by Seth, special code for MFC
   
    bool m_b_vsyc; //added by Seth, to control vsycing
...
public:
    ...
    void SetShowCursor(bool b_new);

    void SetVSync(bool b_new){m_b_vsyc = b_new;}
     HRESULT CD3DApplication::ReinitializeTheDevice(); //added by Seth
    // Internal constructor
    CD3DApplication();
    DWORD GetClearFlags(){return m_clear_flags;}
    void AddClearFlag(DWORD d_new){m_clear_flags |= d_new;}
    void RemoveClearFlag(DWORD d_new){m_clear_flags &= ~d_new;}
    D3DCOLOR GetClearColor(){return 0x00000000;} //to make this app settable easier later
    bool IsWindowed() {return m_d3dpp.Windowed != 0;}
...
protected:
    ...
    BOOL              m_bWindowed;
    BOOL              m_bActive;	  
    bool m_b_cursor_is_on;
    DWORD m_clear_flags;   //do we clear the zbuffer and the screen? 
    bool m_b_remember_windowed; //used when changing modes to remember if we were windowed or not
    bool m_bInitVideoOnNextResize;
